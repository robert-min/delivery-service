...
interface StoreMenuRepository : JpaRepository<StoreMenuEntity, Long> {
    // 유효한 메뉴 체크
    // select * from store_menu where id = ? and status = ? orderby id desc limit 1
    ...
    fun findAllByStoreIdAndStatusOrderBySequenceDesc(store_id: Long?, status: StoreMenuStatus?): List<StoreMenuEntity>
}
